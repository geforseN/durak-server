import { it, todo, describe } from "node:test";
// import { ensureCanDefend } from "../DefenderMoveDefaultBehavior";
  ...
  for (const [weakSuit, unbeatenWeakSuitCards] of weakSuitGropedUnbeatenCards) {
    ...
    if (!defenderWeakCards) {
      ...
      for (const unbeatenWeakSuitCard of unbeatenWeakSuitCards) {
        ...
        if (highPowerUnbeatenWeakSuitCard.length > remainingDefenderTrumpCards.length) {
          throw new Error(
...
describe("it work", () => {
  ...
  it("defend with lowest trump cards", () => {
    assert.doesNotThrow(() => {
